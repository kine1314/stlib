...
void BTree_print(const BTree tree, int layer)
{
    ...
    if (node) {
        __inf("第 %d 层， %d node : ", layer, node->keynum);
...
void BTree_insert(BTree* tree, KeyType key)
{
    ...
    // 根节点已满，插入前需要进行分裂调整,(可以进行非分裂调整，通过跟非满兄弟节点进行合并)
...
{
#ifdef DEBUG_BTREE
	__inf("BTree_merge_child!\n");
#endif
    __ast(tree && node && index >= 0 && index < node->keynum);
    ...
    __ast(leftChild && leftChild->keynum == BTree_D - 1
...
void BTree_recursive_remove(BTree* tree, KeyType key)
{
    ...
    if (!root) {
        __inf("Failed to remove %c, it is not in the tree!\n", key);
        return;
    }
...
void BTree_create(BTree* tree, const KeyType* data, int length)
{
    ...
    for (i = 0; i < length; i++) {
		...
		if (pos!=-1)
		{
			__inf("this key %c is in the B-tree,not to insert.\n",data[i]);
		}else{
